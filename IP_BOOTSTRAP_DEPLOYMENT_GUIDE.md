...
`http://PUBLIC_VPS_IP` for operator testing; HTTP is not encrypted. It does
...
5. Restrict the temporary HTTP endpoint. Recommended Option A is a VPS-provider
   firewall/security-group rule allowing TCP 80 only from `OPERATOR_PUBLIC_IP`.
   ...
   sudo iptables -I DOCKER-USER 2 -i eth0 -p tcp --dport 80 -s "$OPERATOR_PUBLIC_IP" -j ACCEPT
   sudo iptables -I DOCKER-USER 3 -i eth0 -p tcp --dport 80 -j DROP
   ```
   ...
   a reviewer must open a preview, permits TCP 80 publicly at the provider and
   omits the two port-80 `DOCKER-USER` rules. Keep preview TTL at 900 seconds or
...
7. Verify operator HTTP access: `curl --fail http://PUBLIC_VPS_IP/health` and
   `curl --fail http://PUBLIC_VPS_IP/ready`. Generate a normal private preview
